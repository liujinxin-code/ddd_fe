...
// 当前选中子平台的公告内容（来自 subPlatformNotice 字段）
...
const subPlatformOptions = computed(() =>
  subPlatforms.value.map((s) => ({
    ...
    label: s.subPlatformName || `子平台 ${s.subPlatformId}`,
  })),
)
  ...
  <div class="homeIndex-container">
    ...
    <aside class="sidebar-left">
      ...
      <div v-if="subPlatforms.length" class="filter-card">
        <label class="filter-label">子平台</label>
            ...
            <div v-if="services.length" class="service-grid">
              <article v-for="item in services" :key="item.configId" class="service-card" @click="openOrder(item)">
                ...
                <div class="card-footer">
                  <div class="card-meta">
                    <span class="meta-spec">{{ item.priceUnit }}个/包 · {{ item.minQuantity || 1 }}-{{ item.maxQuantity || '不限' }}</span>
...
.card-meta { display: flex; flex-direction: column; gap: 0.1rem; min-width: 0; }
.meta-spec { font-size: 0.74rem; color: $text-muted; }
.meta-price { font-size: 1.1rem; font-weight: 700; color: $color-accent; }
.card-action {
...
.estimated { color: $color-accent; font-weight: 700; text-align: right; font-size: 0.92rem; }
