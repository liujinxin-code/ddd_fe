...
/// <summary>
/// 请求级时区上下文。通过 AsyncLocal 在单个请求的执行流内传递"当前请求时区 ID"（如 "Asia/Shanghai" / "America/New_York"），
/// 由 <see cref="Open.Middlewares.RequestTimeZoneMiddleware"/> 在管道入口设置、出口（finally）清理。
/// 全局 DateTimeOffset 序列化转换器在写出阶段读取此处保存的时区 ID，把数据库统一的上海时间换算成该时区。
///
...
{
    /// <summary>
    /// 当前请求的时区 ID（如 "Asia/Shanghai"）；未设置时为 null，转换器据此按原值（上海时间）输出。
