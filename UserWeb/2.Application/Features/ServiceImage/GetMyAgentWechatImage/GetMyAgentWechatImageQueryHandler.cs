...
namespace Application.Features.ServiceImage
{
    public class GetMyAgentWechatImageQueryHandler(
        ...
        ) : IRequestHandler<GetMyAgentWechatImageQuery, ApiResult<AgentWechatImageResponse>>
    {
        public async Task<ApiResult<AgentWechatImageResponse>> Handle(GetMyAgentWechatImageQuery query, CancellationToken ct)
        {
            ...
            return new ApiResult<AgentWechatImageResponse>
            {
                Code = 200,
                Message = "Success!",
                Data = new AgentWechatImageResponse { ImageUrl = image.ImageUrl, AgentUserid = targetAgentUserId },
                DataTotal = 1
            };
        }
    }
