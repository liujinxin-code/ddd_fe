...
namespace Application.Features.Notice
{
    ...
    /// 分页参数（PageIndex / PageSize）继承自 PagedQuery；页码/页大小已在请求中，响应无需回显，真实总条数由 PagedResult.TotalCount 携带，在 HTTP 边缘层包装为信封 DataTotal。
    /// </summary>
    public record class GetHomepageNoticesQuery() : PagedQuery, IRequest<PagedResult<NoticeResponse>>
    {
    }
}
